...
full_summary_df %>%
  group_by(model, n_plot, design) %>%
  summarize(bias = mean(bias), 
            coverage = mean(coverage), 
            ci_width = mean(ci_width)) %>%
  ungroup() %>%
  arrange(n_plot, design, model) %>%
  print(n = nrow(.))
...
fig_4 <- fig_4_a + fig_4_b + fig_4_c + plot_layout(guides = "collect")
ggsave(file = 'figures/Figure-4.png', width = 15, height = 5, units = 'in',
       bg = 'white')
